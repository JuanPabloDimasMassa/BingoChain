...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Panel de administrador: DOM cargado');
    ...
    if (!form || !submitBtn || !salesStartTimeInput || !salesEndTimeInput || !alertContainer) {
        console.error('Error: No se encontraron algunos elementos del formulario', {
            form: !!form,
            submitBtn: !!submitBtn,
            salesStartTimeInput: !!salesStartTimeInput,
            salesEndTimeInput: !!salesEndTimeInput,
            alertContainer: !!alertContainer
        });
        return;
    }

    console.log('Panel de administrador: Todos los elementos encontrados');
    ...
    // Event listeners para actualizar fecha de fin
    ...
    function showAlert(message, type = 'info') {
        if (!alertContainer) {
            console.error('alertContainer no está disponible');
    ...
    async function createLottery(event) {
        ...
        console.log('Enviando petición para crear sorteo:', lotteryData);
        ...
        try {
            ...
            if (response.ok) {
                showAlert(
                    ...
                    <strong>Fecha de creación:</strong> ${new Date(data.createdAt).toLocaleString('es-ES')}`,
                    'success'
                );
                
                console.log('Sorteo creado exitosamente con ID:', data.id);
            ...
            showAlert(
                '❌ Error de conexión. Por favor verifica que el servidor esté corriendo en http://localhost:3500',
                'error'
            );
        } finally {
            setLoading(false);
    ...
    if (contractAddressInput) {
        contractAddressInput.addEventListener('input', function(e) {
